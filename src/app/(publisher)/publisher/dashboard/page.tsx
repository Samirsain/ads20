        ...
        <div className="bg-slate-900/40 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 lg:col-span-2 space-y-4">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/50 text-slate-300 text-sm">
                {loading ? (
                  ...
                  <tr><td className="px-4 py-8 text-center text-slate-500" colSpan={4}>No conversions recorded yet.</td></tr>
                ) : data.recentConversions.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-800/20 transition-colors">
                    <td className="px-4 py-3 font-mono text-xs text-slate-400">{c.externalUserId}</td>
                    <td className="px-4 py-3">
                      <span className="font-mono bg-slate-950 px-2 py-0.5 rounded border border-slate-800 text-blue-400 text-xs">{c.trackingLink.uniqueCode}</span>
                    </td>
                    <td className="px-4 py-3 font-medium text-emerald-400">${Number(c.amount).toFixed(2)}</td>
          ...
          <div className="md:hidden space-y-2">
            {loading ? (
              ...
              <p className="text-center text-slate-500 py-6 text-sm">No conversions recorded yet.</p>
            ) : data.recentConversions.map((c) => (
              <div key={c.id} className="bg-slate-950/60 border border-slate-800 rounded-xl p-3 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-mono bg-slate-900 border border-slate-800 px-2 py-0.5 rounded text-blue-400 text-xs">{c.trackingLink.uniqueCode}</span>
                    <span className="font-mono font-bold text-emerald-400 text-sm">${Number(c.amount).toFixed(2)}</span>
                  </div>
                  <p className="font-mono text-xs text-slate-500 truncate">{c.externalUserId}</p>
                </div>
                <span className="text-slate-600 text-xs shrink-0">{new Date(c.createdAt).toLocaleDateString()}</span>
              </div>
            ))}
          </div>
        </div>
