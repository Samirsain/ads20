...
const NETWORKS = [
  { value: 'TRC20', label: 'TRC20 (Tron)', color: 'text-red-400' },
  { value: 'ERC20', label: 'ERC20 (Ethereum)', color: 'text-blue-400' },
  { value: 'BEP20', label: 'BEP20 (BSC)', color: 'text-amber-400' },
]
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-400 mt-1">Request a USDT payout to your crypto wallet</p>
      </div>
      ...
      <div className="flex items-start gap-3 p-4 rounded-2xl bg-amber-500/8 border border-amber-500/20">
        ...
        <div>
          ...
          <p className="text-amber-400/70 text-xs mt-0.5">Next payout date: <span className="font-medium text-amber-300">{getNextMonday()}</span></p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <div className="bg-slate-900/40 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 shadow-2xl sticky top-8 space-y-5">
            <h3 className="text-xl font-bold text-white">Request Payout</h3>

            {formError && (
              <div className="p-3 rounded-xl bg-red-950/30 border border-red-800/50 text-red-400 text-sm">
                {formError}
              </div>
            )}
            {successMsg && (
              <div className="p-3 rounded-xl bg-emerald-950/30 border border-emerald-800/50 text-emerald-400 text-sm flex items-center gap-2">
            ...
            <form onSubmit={handleWithdraw} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950/60 border border-slate-800 focus:border-blue-500 rounded-xl pl-7 pr-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-blue-500 transition font-mono"
                    placeholder="50.00"
                  />
                </div>
                <p className="text-xs text-slate-500 mt-1.5">Minimum: $50.00 USDT</p>
              </div>
              ...
              {/* Wallet Address */}
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1.5">USDT Wallet Address</label>
                <input
                  type="text"
                  required
                  value={walletAddress}
                  onChange={(e) => setWalletAddress(e.target.value)}
                  className="w-full bg-slate-950/60 border border-slate-800 focus:border-blue-500 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-blue-500 transition font-mono text-xs"
              ...
              <div className="flex items-start gap-2 p-3 rounded-xl bg-slate-950/40 border border-slate-800">
                ...
                <p className="text-slate-400 text-xs">Payment will be sent on next Monday after admin approval. Make sure your address and network match exactly.</p>
              </div>

              <button
                type="submit"
                disabled={requesting}
                className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-medium rounded-xl py-3 shadow-lg shadow-blue-600/15 hover:shadow-blue-600/25 active:scale-[0.98] transition disabled:opacity-50"
              >
                {requesting ? (
        ...
        <div className="lg:col-span-2">
          <div className="bg-slate-900/40 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 shadow-2xl">
            <h3 className="text-xl font-bold text-white mb-5">Payout History</h3>
                ...
                {withdrawals.map((w) => (
                  <div key={w.id} className="bg-slate-950/60 border border-slate-800 rounded-2xl p-4 hover:border-slate-700 transition-colors">
                    <div className="flex flex-wrap items-center gap-3 mb-2">
                      ...
                      <span className="text-xs font-bold px-2 py-0.5 rounded-md bg-slate-800 border border-slate-700 text-slate-300">
                        {w.network}
                      </span>
                      {w.status === 'PENDING' && (
                    ...
                    <div className="flex items-center gap-2">
                      <p className="font-mono text-xs text-slate-400 truncate flex-1">{w.walletAddress}</p>
                      <button
