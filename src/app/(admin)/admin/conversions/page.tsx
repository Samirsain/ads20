    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">View all publisher conversions globally</p>
        </div>
        <button
              ...
              {loading ? (
                ...
                conversions.map((conv) => (
                  <tr key={conv.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 font-mono text-xs text-slate-400">{conv.id}</td>
                    <td className="px-6 py-4">
                      <div className="font-semibold text-slate-900">{conv.publisher?.name || 'N/A'}</div>
                      <div className="text-slate-400 text-xs mt-0.5">{conv.publisher?.email || 'N/A'}</div>
                    </td>
                    <td className="px-6 py-4 font-mono text-slate-500">{conv.trackingLink?.uniqueCode || 'N/A'}</td>
                    <td className="px-6 py-4 font-mono text-slate-500">{conv.externalUserId}</td>
